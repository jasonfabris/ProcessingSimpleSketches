    ...
    int cpY2 = int(random(height));
     ...
     for(int i = 0; i<1000; i++) {
       stroke(jellyfish[int(random(5))],jellyfish[int(random(5))],jellyfish[int(random(5))], 40);
          ...
          float xadj = (randomGaussian()*(width/100));
          float yadj = (randomGaussian()*(height/100));
          
       beginShape();
         vertex(random(width/2), random(height/2 + yadj)); // for the first point!
         bezierVertex(cpX1, cpY1, cpX2, cpY2, x, y);
         bezierVertex(cpX2, cpY2, cpX1, cpY1, y, x);
         println(cpX1, ":", cpY1, ":", cpX2, ":", cpY2, ":", x, y);
       endShape();
