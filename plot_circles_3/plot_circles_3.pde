...
void setup() {
    ...
    shps = new ArrayList<Circ>();
     ...
     for (int shp = 0; shp < num_shps; shp++) {  
        float rad_step = map(shp % 9, 0, 8, .1, 1); // every third circle
        float r2 = base_r * rad_step;
        Circ c = new Circ(25, r2, shp/20);
        shps.add(c);
...
void draw() {
  ...
  if (record) {
    beginRecord(PDF, String.format("C:/Users/Jason/Documents/Processing Projects/Output/plot_circs_3_%d%d%d%d%d.pdf", year(), month(), day(), hour(), minute(), second()));
  }   
