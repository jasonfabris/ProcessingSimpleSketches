class OscAttr {
  ...
  void display() {
    ...
    //r = amp * cos(TWO_PI * (period*period) / log(frameCount));
    r = amp * cos(frameCount / period);
    
    println(r, frameCount);
    
    noFill();
    stroke(80, 90, 100, 100);
    ellipse(loc_x, loc_y, r, r);
    
    //float r_old = amp * cos(period/(frameCount-1));
    //float r_old = amp * sin((period)/log(sqrt(frameCount)));
    float r_old = amp * cos((frameCount/2) / period);
    noFill();
    stroke(275, 25, 80, 100);
