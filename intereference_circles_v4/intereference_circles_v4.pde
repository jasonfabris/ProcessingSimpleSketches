...
beginRecord(PDF, String.format("C:/Users/Jason/Documents/Processing Projects/Output/interference_pattern4_%d%d%d%d%d.pdf", year(), month(), day(), hour(), minute(), second()));
...
color b = color(90, 100, 18);
color l = color(300, 90, 100);
...
//build array of centres
    ...
    for(int cx = 0; cx < r; cx++) {
            cent_x = width/2 + map(cx, 0, r-1, -150, 150);
        for(int cy = 0; cy < c; cy++) {
            cent_y = width/2 + map(cy, 0, c-1, -150, 150);
